...
public sealed class BloodCultRuleSystem : GameRuleSystem<BloodCultRuleComponent>
{
    ...
    // !HOTFIX - O sistema de speak foi refatorado para usar OnActionSpeak (componente posto em entidades de ações pra falar algo quandoa usa.
    // porem o sistema do blood cult nao parece funcionar com esse sisitema.
    public void Speak(EntityUid? uid, string speech)
    {
        if (uid is null || string.IsNullOrWhiteSpace(speech))
            return;

        if (!HasComp<SpeechComponent>(uid) || HasComp<MutedComponent>(uid))
            return;

        _chat.TrySendInGameICMessage(uid.Value, Loc.GetString(speech), InGameICChatType.Speak, false);
    }
