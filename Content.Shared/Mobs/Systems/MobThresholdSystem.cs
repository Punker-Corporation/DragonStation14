...
public sealed partial class MobThresholdSystem : EntitySystem
{
    ...
    /// <summary>
    /// Replaces all mob-state thresholds at once and re-validates the target's current state.
    /// </summary>
    ...
    {
        if (!Resolve(target, ref thresholdComponent, ref mobState, ref damageable))
            return;

        thresholdComponent.Thresholds = new SortedDictionary<FixedPoint2, MobState>(thresholds);
        Dirty(target, thresholdComponent);
        VerifyThresholds(target, thresholdComponent, mobState, damageable, body);
    }
